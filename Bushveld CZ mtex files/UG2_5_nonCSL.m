...
csl = CSL(sigma(i),cs);
if length(csl) == 1
 l(i) = sum(gbc(angle(gbc.misorientation,csl)<sqrt(sigma(i))*8*degree).segLength)
else
    clear ltempj
    for j =1:length(csl)
        ltempj(j) = sum(gbc(angle(gbc.misorientation,csl(j))<sqrt(sigma(i))*8*degree).segLength)
    end
    l(i) = sum(ltempj)
end
end
...
% gbc_n =  ~ gbcC; -not working 

gbc_n = gbc(angle(gbc.misorientation)<55/sqrt(3)*degree); 
...
condition = gB_ChrChr.misorientation.angle./degree<55 
plot(grains(condition).boundary);
...
hold on
[~,mP] = plot(gbcC,'lineColor',[0 1 0.3], 'linewidth',2);
hold off
