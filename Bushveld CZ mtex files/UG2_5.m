...
if length(csl) == 1
 l(i) = sum(gbc(angle(gbc.misorientation,csl)<sqrt(sigma(i))*5*degree).segLength)
    ...
    clear ltempj
    for j =1:length(csl)
        ltempj(j) = sum(gbc(angle(gbc.misorientation,csl(j))<sqrt(sigma(i))*5*degree).segLength)
    end
    l(i) = sum(ltempj)
end
end
...
% test
figure
plot(g1,'FaceColor',[1 0 0])
hold on
plot(g2,'FaceColor',[0 0 1])
hold on
plot(gbcC,'lineColor',[0 1 0.3], 'linewidth',1)
hold off
